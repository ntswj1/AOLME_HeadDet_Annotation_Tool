        ...
        if os.path.exists(self.labelfilename):
            with open(self.labelfilename) as f:
                for (i, line) in enumerate(f):
                    ...
                    print(classes[int(yolo_data[0])])
        
    def saveImage(self):
        with open(self.labelfilename, 'w') as f:
            #print(self.bboxList)
            #print(self.bboxListCls)
            #print(self.labelname)
            #for bbox,bboxcls in zip(self.bboxList,self.bboxListCls):
            for bbox,bboxcls,llabel in zip(self.bboxList,self.bboxListCls, self.labelname):
                xmin,ymin,xmax,ymax = bbox
                b = (float(xmin), float(xmax), float(ymin), float(ymax))
                bb = self.convert((self.curimg_w,self.curimg_h), b)
                
# =============================================================================
#                 x = bb[0] * self.curimg_w
#                 y = bb[2] * self.curimg_h
#                 w = bb[1] - x
#                 h = bb[3] - y
# =============================================================================
                #tmp = self.deconvert(yolo_data[1:])
                #print('bboxcls', bboxcls)
                #f.write(str(bboxcls) + " " + " ".join([str(a) for a in bb])  + '\n')
                f.write(str(bboxcls) + " " + " ".join([str(a) for a in bb]) + " " + llabel + '\n')
    ...
    def mouseClick(self, event):
        if self.STATE['click'] == 0:
            ...
            self.labelname.append(classes[self.cur_cls_id])
            self.bboxId = None
    ...
    def delBBox(self):
        ...
        print('pop', self.bboxListCls,idx)
        self.bboxListCls.pop(idx)
        self.labelname.pop(idx)
